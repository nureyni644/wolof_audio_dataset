...
def list_videosByChannel(channel_url):
    ...
    if result is not None:
        for entry in result.get('entries', []):
            if eval("FILTER_KEYWORDS") == []:
                video_urls.append(entry.get('url'))
            else:
                for keyWord in eval("FILTER_KEYWORDS"):
...
@dag(
    dag_id='url_videos_CHANNEL_NAME',
...
def yt_videos_dag():
    ...
    task_list_videos = process_channel(channel_url='YOUTUBE_CHANNEL_URL')
