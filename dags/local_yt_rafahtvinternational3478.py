    ...
    if result is not None:
        for entry in result.get('entries', []):
            if eval("['khoutba']") == []:
                ...
                for keyWord in eval("['khoutba']"):
                    if  keyWord.lower() in entry.get('title','').lower():
                        print(f"Checking for keyword: {keyWord.lower()}")
                        video_urls.append(entry.get('url'))
                        break
                    # print(f"Keyword '{keyWord}' not found in title.")
    print(f"Found {len(video_urls)} videos in channel.")
    return video_urls


@dag(
    dag_id='url_videos_rafahtvinternational3478',
    default_args=default_args,
    description='List YouTube videos from a channel and save their URLs locally',
    schedule='@daily',
...
def yt_videos_dag():
    ...
    task_list_videos = process_channel(channel_url='https://www.youtube.com/@rafahtvinternational3478/videos')
